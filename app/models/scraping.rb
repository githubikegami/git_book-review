class Scraping
  ...
  def self.movie_urls
    ...
    abc = []
    agent = Mechanize.new
    current_page = agent.get("https://books.rakuten.co.jp/search?sitem=python&g=001&l-id=pc-search-box&x=0&y=0")
  ...
  def self.get_product(link)
    ...
    publisher = page.at('//*[@id="productDetailedDescription"]/div/ul/li[2]/span[2]/a[1]').inner_text
    # ok
    description = page.at('//*[@id="editArea2"]/div/p[2]/text()').inner_text
    # ok
    
    bookreview = Product.new(image_url: image_url, title: title, price: price,release_date: release_date,author: author,publisher: publisher ,description: description)
    bookreview.save
  end
end
